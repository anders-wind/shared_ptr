...
TEST_SUITE("biased::shared_ptr")
{
    TEST_CASE("biased::shared_ptr: make_shared works")
    {
        auto my_shared = wind::biased::make_shared<int>(42);
        CHECK(*my_shared == 42);
        CHECK(my_shared.get_local_count() == 1);
    }

    TEST_CASE("biased::shared_ptr: copy operator= works")
    {
        auto my_shared = wind::biased::make_shared<int>(42);
        auto copy = my_shared;
        CHECK((*copy) == (*my_shared));
        CHECK(my_shared.get_local_count() == 2);
        CHECK(copy.get_local_count() == 2);
    }

    TEST_CASE("biased::shared_ptr: move operator= works")
    {
        auto my_shared = wind::biased::make_shared<int>(42);
        auto moved = std::move(my_shared);
        CHECK(moved.get_local_count() == 1);
    }

    TEST_CASE("biased::shared_ptr: Destructor does not delete when copy exists")
    {
        wind::biased::shared_ptr<int> out_copy;
        {
            auto my_shared = wind::biased::make_shared<int>(42);
            out_copy = my_shared;
            CHECK(out_copy.get_local_count() == 2);
        }
        CHECK(out_copy.get_local_count() == 1);
        CHECK(*out_copy.data == 42);
    }
    ...
    TEST_CASE("biased::shared_ptr: Delete gets called")
    {
        bool was_called = false;
        {
            auto ptr =
                wind::biased::make_shared<DeleterFunc>([&was_called]() { was_called = true; });
        }
        CHECK(was_called);
    }

    TEST_CASE("biased::shared_ptr: Delete gets called when 2 copies")
    {
        bool was_called = false;
        {
            auto ptr =
    ...
    TEST_CASE("biased::shared_ptr: Delete only gets called on last destructor")
    {
        bool was_called = false;
        {
            auto ptr =
