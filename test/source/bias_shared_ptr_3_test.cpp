...
TEST_SUITE("bias_3::shared_ptr")  // NOLINT
{
    TEST_CASE("bias_3::shared_ptr: nullptr destruction is ok for copies")  // NOLINT
    {
        wind::bias_3::shared_ptr<int> val;

        auto copy = val;  // NOLINT

        wind::bias_3::shared_ptr<int> operator_copy;
        operator_copy = copy;

        auto init_operator_copy = wind::bias_3::make_shared<int>(42);
    ...
    TEST_CASE("bias::shared_ptr_3: nullptr destruction is ok for moves")  // NOLINT
    {
        ...
        wind::bias_3::shared_ptr<int> operator_move;
        operator_move = std::move(copy);

        auto init_operator_move = wind::bias_3::make_shared<int>(42);
        init_operator_move = std::move(operator_move);
    }

    TEST_CASE("bias::shared_ptr_3: copy constructors work")  // NOLINT
    {
        auto val = wind::bias_3::make_shared<int>(42);
        ...
        auto init_operator_copy = wind::bias_3::make_shared<int>(3);
        init_operator_copy = operator_copy;
        CHECK(*operator_copy == 42);
    ...
    {
        auto value = wind::bias_3::make_shared<int>(0);
        CHECK(*value == 0);
        (*value)++;
        CHECK(*value == 1);
    }
    ...
    {
        auto value1 = wind::bias_3::make_shared<int>(1);
        (*value1)++;
        auto value2 = wind::bias_3::make_shared<int>(42);
        CHECK(*value1 == 2);
        CHECK(*value2 == 42);
    }
    ...
    {
        auto value1 = wind::bias_3::make_shared<int>();
        *value1 = 1;
        auto copy = value1;
        CHECK(*value1 == 1);
        CHECK(*copy == 1);
    ...
    TEST_CASE("bias::shared_ptr_3: destructor gets")  // NOLINT
    {
        ...
        {
            auto value = wind::bias_3::make_shared<deleter_ref>();
    ...
    {
        auto ptrs = std::vector<wind::bias_3::shared_ptr<int>>();
        for (auto i = 0; i < 2; i++) {
            ptrs.push_back(wind::bias_3::make_shared<int>(i + 1));
        }
        CHECK(*ptrs[0] == 1);
        CHECK(*ptrs[1] == 2);

        auto default_ptrs = std::vector<wind::bias_3::shared_ptr<int>>(2);
    ...
    {
        auto ptr = wind::bias_3::make_shared<int>(3);
        auto copy = ptr;
        auto copy2 = ptr;
        copy = copy2;
        CHECK(*ptr == 3);
    ...
    {
        auto empty = wind::bias_3::shared_ptr<int>();
    ...
    {
        {
            ...
            for (int64_t i = 0; i < number_of_push_backs_until_resize - 1; i++) {
                ptrs.push_back(wind::bias_3::make_shared<int>(static_cast<int>(42 * i)));
            }
            ptrs.push_back(wind::bias_3::make_shared<int>(static_cast<int>(-2)));
    ...
    {
        auto ptr = wind::bias_3::make_shared<int>(static_cast<int>(42));
        auto ptrs = std::vector<wind::bias_3::shared_ptr<int>>();
        const auto number_of_push_backs_until_resize = 1024;
        for (int64_t i = 0; i < number_of_push_backs_until_resize - 1; i++) {
            ptrs.push_back(ptr);
